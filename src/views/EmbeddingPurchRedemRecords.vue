	...
	<section>
		...
		<el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
			<el-form :inline="true" :model="filters">
				...
				<el-form-item>
					<el-button type="primary" v-on:click="getEmbeddingPurchRedemRecords">查询</el-button>
				</el-form-item>
				<el-form-item>
		...
		<el-table :data="embedding_purch_redem_records" highlight-current-row v-loading="listLoading" @selection-change="onSelectionChanged"
		 @sort-change="onSortChanged" style="width: 100%;">
			...
			<el-table-column prop="security.prod_name" label="标的名称" width="160" sortable="custom">
			</el-table-column>
...
<script>
	...
	import {
		getEmbeddingPurchRedemRecordsPage,
		addEmbeddingPurchRedemRecord,
		editEmbeddingPurchRedemRecord,
		deleteEmbeddingPurchRedemRecord,
		deleteEmbeddingPurchRedemRecords,
		getAccountsPage,
	} from '../api/api';
	...
	export default {
		...
		methods: {
			...
			getEmbeddingPurchRedemRecords: function() {
				let para = {
					page: this.page,
				...
				//NProgress.start();
				getEmbeddingPurchRedemRecordsPage(para)
					.then((res) => {
						this.total = res.data.total;
						this.embedding_purch_redem_records = res.data.data;
			...
			handleDel: function(index, row) {
				this.$confirm('确认删除该记录吗?', '提示', {
					...
					deleteEmbeddingPurchRedemRecord(para)
						.then((response) => {
							this.listLoading = false;
							//NProgress.done();
							this.$message({
				...
				this.$refs.editForm.validate((valid) => {
					if (valid) {
						this.$confirm('确认提交吗？', '提示', {}).then(() => {
							...
							editEmbeddingPurchRedemRecord(para)
								.then((response) => {
				...
				this.$refs.addForm.validate((valid) => {
					if (valid) {
						this.$confirm('确认提交吗？', '提示', {}).then(() => {
							...
							para.trade_date = this.formatDate(para.trade_date);
							addEmbeddingPurchRedemRecord(para)
								.then((response) => {
